...
classdef AutoMATiC_Generator_osQP < handle
    properties (Constant)
    ...
    methods
        function obj = AutoMATiC_Generator_osQP(toexecute,tree) 
...
%                             tcode = [tcode, sprintf('%s = dtablica3(1,%d,1,%d,1,%d);',m{1},msize(1),msize(2),msize(3)), newline ];
            ...
            if(isfield(obj.qpconf,'Hid'))
                ...
                [dddA,cccA,nzelA]=generateCSC(tmpmatrix, mid);
                
                mid = obj.qpconf.Hid;
                tmpmatrix = evalin('base', [ mid ';']);
                [dddH,cccH,nzelH]=generateCSC(tmpmatrix, mid);
                ...
                code = [code,...
                    dddA, newline,...
                    dddH, newline,...
                    sprintf('static OSQPSettings *%s;',obj.qpconf.settingsid), newline,...
                    sprintf('static OSQPWorkspace *%s;',obj.qpconf.workid), newline,...
                    sprintf('static OSQPData *%s;',obj.qpconf.dataid), newline,...
                    sprintf('static c_float %s[%d];',obj.qpconf.qid, Aidsize(2)), newline,...
                    sprintf('static c_float %s[%d];',obj.qpconf.lbid, Aidsize(1)), newline,...
                    sprintf('static c_float %s[%d];',obj.qpconf.ubid, Aidsize(1)), newline,...
                    sprintf('static c_float %s[%d];',obj.qpconf.fid, Aidsize(2)+1),newline,...
...                    sprintf('static c_float %s[%d];',obj.qpconf.bid, 2*Aidsize(1)+1),newline,...
                    sprintf('static c_float %s[%d];',obj.qpconf.bid, Aidsize(1)+1),newline,... % !!!
                    sprintf('static c_float %s[%d];',obj.qpconf.xid, Aidsize(2)+1),newline,...
                ];
                qpcode = [...
                      cccA, newline,...
                      cccH, newline,...
                      sprintf('%s = (OSQPSettings *)c_malloc(sizeof(OSQPSettings));',obj.qpconf.settingsid),newline,...
                      sprintf('%s = (OSQPData *)c_malloc(sizeof(OSQPData));',obj.qpconf.dataid),newline,...
                      sprintf('for(int osqp_it=0; osqp_it < %d; ++osqp_it) %s[osqp_it] = -100000;',Aidsize(1),obj.qpconf.lbid),newline,...
                      sprintf('%s->n = %d;',obj.qpconf.dataid,Aidsize(2)),newline,...
                      sprintf('%s->m = %d;',obj.qpconf.dataid,Aidsize(1)),newline,...
                      sprintf('%s->q = %s;',obj.qpconf.dataid,obj.qpconf.qid),newline,...
                      sprintf('%s->l = %s;',obj.qpconf.dataid,obj.qpconf.lbid),newline,...
                      sprintf('%s->u = %s;',obj.qpconf.dataid,obj.qpconf.ubid),newline,...
                      sprintf('%s->P = csc_matrix(%s->n, %s->n, %d, %s_x, %s_i, %s_p);',obj.qpconf.dataid,obj.qpconf.dataid,obj.qpconf.dataid,nzelH,obj.qpconf.Hid,obj.qpconf.Hid,obj.qpconf.Hid),newline,...
                      sprintf('%s->A = csc_matrix(%s->m, %s->n, %d, %s_x, %s_i, %s_p);',obj.qpconf.dataid,obj.qpconf.dataid,obj.qpconf.dataid,nzelA,obj.qpconf.Aid,obj.qpconf.Aid,obj.qpconf.Aid),newline,...
                      sprintf('osqp_set_default_settings(%s);',obj.qpconf.settingsid),newline,...
                      sprintf('%s = osqp_setup(%s, %s);',obj.qpconf.workid,obj.qpconf.dataid,obj.qpconf.settingsid),newline
                ];
...
%                             sprintf('iloczynatb(%s,%s,%s,%d,%d,%d,%d);',lhsid,rhsid,target,lhssize(1),lhssize(2),rhssize(1),rhssize(2))...
        ...
        function code = generateQuadprogAssignment(obj,node)
            ...
            obj.qpconf.workid = 'osqp_work';
            obj.qpconf.dataid= 'osqp_data';
            obj.qpconf.settingsid = 'osqp_settings';
            obj.qpconf.qid = 'osqp_q';
            obj.qpconf.lbid = 'osqp_lb';
            obj.qpconf.ubid = 'osqp_ub';

            Aidexist = evalin('base',['exist(''' obj.qpconf.Aid ''');']);
            if(Aidexist)
...                evalin('base',[obj.qpconf.Aid ' = -' obj.qpconf.Aid '(1:(end/2),:);']);
                Aidsize = evalin('base',['size(' obj.qpconf.Aid ');']);
            else
                error('A matrix is not initialized -- this one must be a constant');
            end
            ...
            code = [
                sprintf('memcpy (%s, &(%s[1]), %d*sizeof(c_float));',obj.qpconf.qid, obj.qpconf.fid, Aidsize(2)),newline,...
                sprintf('osqp_update_lin_cost(%s, %s);',obj.qpconf.workid, obj.qpconf.qid),newline,...
...                 sprintf('memcpy (%s, &(%s[1+%d]), %d*sizeof(c_float));',obj.qpconf.ubid, obj.qpconf.bid, Aidsize(1),Aidsize(1)),newline,...
...                 sprintf('productv((float *)%s,-1,(float *)%s,%d);',obj.qpconf.bid,obj.qpconf.bid,Aidsize(1)*2),newline,...
...                 sprintf('memcpy (%s, &(%s[1+%d]), %d*sizeof(c_float));',obj.qpconf.lbid, obj.qpconf.bid,         0 ,Aidsize(1)),newline,...
                sprintf('memcpy (%s, &(%s[1]), %d*sizeof(c_float));',obj.qpconf.ubid, obj.qpconf.bid, Aidsize(1)),newline,... % !!!
                sprintf('osqp_update_bounds(%s, %s, %s);',obj.qpconf.workid, obj.qpconf.lbid, obj.qpconf.ubid), newline,...
                sprintf('osqp_solve(%s);',obj.qpconf.workid),newline,...
                sprintf('memcpy (&(%s[1]), %s->solution->x, %d*sizeof(c_float));',obj.qpconf.xid, obj.qpconf.workid,Aidsize(2)),newline,...
                newline,...
            ];            
        end        
...
function [d,c,nnz] = generateCSC(M,name)
    n = size(M,1);
    m = size(M,2);
    x = [];
    ind = [];
    ...
    d1 = sprintf('static c_float %s_x[%d];\n',name, numel(x));
    d2 = sprintf('static c_int %s_i[%d];\n',name, numel(ind));
    d3 = sprintf('static c_int %s_p[%d];\n',name, numel(p));
    d = [d1,d2,d3];
    c1 = sprintf(sprintf('%s_x[%%d] = %%ef;\\n',name), [0:length(x)-1;x(:)']);
    c2 = sprintf(sprintf('%s_i[%%d] = %%d;\\n',name), [0:length(ind)-1;ind(:)']);
    c3 = sprintf(sprintf('%s_p[%%d] = %%d;\\n',name), [0:length(p)-1;p(:)']);
    c = [c1,c2,c3];
end
